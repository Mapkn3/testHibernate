package entities;

import javax.persistence.*;

@Entity
@Table(name = "SPELL", schema = "TEST")
public class SpellEntity extends PrettyEntity {
    private long id;
    private String name;
    private String description;
    private PersonageClassEntity personageClass;

    @Id
    @Column(name = "SPELLID")
    public long getId() {
        return id;
    }

    public void setId(long id) {
        this.id = id;
    }

    @Basic
    @Column(name = "SPELLNAME")
    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    @Basic
    @Column(name = "SPELLDESCRIPTION")
    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;

        SpellEntity that = (SpellEntity) o;

        if (id != that.id) return false;
        if (name != null ? !name.equals(that.name) : that.name != null) return false;
        if (description != null ? !description.equals(that.description) : that.description != null)
            return false;

        return true;
    }

    @Override
    public int hashCode() {
        int result = (int) (id ^ (id >>> 32));
        result = 31 * result + (name != null ? name.hashCode() : 0);
        result = 31 * result + (description != null ? description.hashCode() : 0);
        return result;
    }

    @ManyToOne
    @JoinColumn(name = "FORCLASSID", referencedColumnName = "PERSONAGECLASSID", nullable = false)
    public PersonageClassEntity getPersonageClass() {
        return personageClass;
    }

    public void setPersonageClass(PersonageClassEntity personageClass) {
        this.personageClass = personageClass;
    }

    @Override
    public long primaryKey() {
        return id;
    }
}
